...
import { executionFields as fields, executionHash as hash, executionId as id, executionText as text, parseExecutionJson } from './execution-contracts.js'
...
export function normalizeGripperCheck(value) {
  fields(value, ['contractVersion', 'nodeSessionId', 'configuration', 'inspection', 'trial', 'canInspect', 'canPrepare', 'canApprove', 'canStop', 'blockedReason'])
...
export const commissioningUnresolved = (status) => Boolean(status?.trial && (!['COMPLETED', 'STOPPED', 'FAILED'].includes(status.trial.phase) || status.trial.stopStatus !== 'STOPPED'))
...
export function createCommissioningClient({ baseUrl, token, fetchImpl = globalThis.fetch } = {}) {
  ...
  async function request(action, body) {
    ...
    try {
      ...
      const response = await fetchImpl(url, { method: body === undefined ? 'GET' : 'POST', redirect: 'error', cache: 'no-store', signal: AbortSignal.timeout(action === 'inspect' ? 35000 : 5000),
