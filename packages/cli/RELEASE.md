...
1. Uses `npm run release:prepare` to build `physicalsystems@0.2.6` once on
...
6. Verifies the inert namespace bootstrap, the already-published runtime, and
   that `physicalsystems@0.2.6` is not already public. This update requires the
   exact existing tags `bootstrap=0.0.0`, `latest=0.0.0`, and `preview=0.2.5`;
   unexpected tag changes block publication rather than being overwritten.
7. Publishes exactly one tarball:

   ```bash
   npm publish "./release-artifacts/physicalsystems-0.2.6.tgz" \
     --registry="https://registry.npmjs.org/" \
     --provenance --tag preview --access public
   ```

8. Confirms `bootstrap=0.0.0`, `latest=0.0.0`, and `preview=0.2.6`; compares
...
```bash
npm view physicalsystems@0.2.6 version dist.integrity dist.shasum dist.attestations --json
npm view physicalsystems dist-tags --json
npx --yes physicalsystems@0.2.6 --version
npm install --global physicalsystems@0.2.6
physicalsystems
```

Also create an empty audit directory, install with
`npm install --ignore-scripts --no-audit --no-fund physicalsystems@0.2.6`, and
...
```bash
npm dist-tag add physicalsystems@0.2.6 latest
```
