...
test('desktop host shares the reviewed allowlist, isolates resources and persists an empty conversation', async (t) => {
  ...
  assert.match(fake.calls.resourceOptions.systemPrompt, /Review experiment card and its Approve & continue button/)
  assert.match(fake.calls.resourceOptions.systemPrompt, /Ordinary prose consent does not approve an experiment/)
  assert.match(fake.calls.resourceOptions.systemPrompt, /Experiments tab in the right panel is an optional detailed view/)
