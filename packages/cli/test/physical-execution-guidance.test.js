...
test('physical prompt describes the current operator execution path after inspecting its availability', () => {
  ...
  assert.match(prompt, /no physical preparation, approval, dispatch, stop or reconciliation tools/)
  assert.match(prompt, /the model has no approval tool/)
  assert.match(prompt, /Do not silently substitute this fixture for a requested hardware experiment/)
