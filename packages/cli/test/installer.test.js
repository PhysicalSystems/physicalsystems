...
test('the staged Windows bootstrap pins and verifies every downloaded runtime', () => {
  ...
  assert.match(installer, /This unreleased-source installer is Windows-only/)
  assert.match(installer, /npm view tinyedge@0\.1\.5 version --json/)
  assert.match(installer, /npx --yes tinyedge@0\.1\.5/)
