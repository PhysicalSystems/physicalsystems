...
import { cameraIsFresh, executionReadIsFresh, executionApprovalAvailable } from '../../../cli/src/harness/workcell-view/view-state.js'
...
export async function view(t, options = {}) {
  ...
  const source = readFileSync(new URL('../../src/renderer/workcell.js', import.meta.url), 'utf8')
    .replace(/^import .* from .*\r?\n/m, '').replace('export function mountWorkcellView', 'function mountWorkcellView')
  let controller
  runInNewContext(source + '\nsetController(mountWorkcellView(root, { command }))', {
    ...
    URL: { createObjectURL(blob) { assert.equal(blob.type, 'image/jpeg'); const url = `blob:test-${++nextUrl}`; created.push(url); return url }, revokeObjectURL(url) { revoked.push(url) } },
    Blob, Uint8Array, AbortController, AbortSignal, Date: ClockDate,
    ...
    command: async (name, payload) => {
      const pending = deferred()
      const collection = name === 'workcell.camera.frame' ? requests : actions
      collection.push({ name, payload, ...pending })
  ...
  return {
    ...
    async push(next) { state = { ...next, revision: state.revision + 1 }; controller.update(state); await tick(); await tick() },
    async pushCamera(value) { await this.push({ ...state, camera: value }) },
    async response(request = requests.at(-1), fail = false) {
      assert.ok(request, 'a frame request is pending')
      if (fail) request.reject(new Error('Exact frame unavailable'))
      else request.resolve({ id: request.payload.frameId, contentType: 'image/jpeg', bytes: [1,2,3] })
      await tick(); await tick()
    },
    ...
    async disconnect() { controller.update(state, false); await tick(); await tick() },
    async close() { controller.dispose(); await tick(); await tick() },
  }
}
