...
test('workflow drift fails before creating outputs or invoking preparation', async () => {
  await withFixture(async (fixture) => {
    ...
    await fs.writeFile(filename, workflow.replace('RELEASE_VERSION: ${{ needs.require-main.outputs.version }}', 'RELEASE_VERSION: 9.9.9'))
    let prepared = false
...
test('consumer toolchain drift is rejected; tags are read from the descriptor', async () => {
  for (const mutate of [
